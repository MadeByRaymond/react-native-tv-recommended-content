...
[![npm version](https://img.shields.io/npm/v/react-native-tv-recommended-content.svg)](https://www.npmjs.com/package/react-native-tv-recommended-content)
[![license](https://img.shields.io/npm/l/react-native-tv-recommended-content.svg)](./LICENSE)
...
At the time of writing, there is no existing, actively maintained React Native library for publishing to the Android TV Watch Next row or managing custom home screen channels — both are otherwise only reachable by writing raw Kotlin against `androidx.tvprovider` yourself. This package wraps that entire surface with a single, fully-typed JS/TS API.
...
| iOS / tvOS | Same safe no-op behavior. There is no tvOS equivalent of Watch Next/Channels (tvOS has its own separate, unrelated "Top Shelf" API), so no native implementation exists or is planned for this package |
...
**The default channel:** the very first channel your app creates is automatically treated by the system as browsable — it appears on the home screen immediately, with no user approval needed. Every channel after that requires the user to explicitly approve adding it (the system shows a permission prompt when you call `createChannel`). This package tracks which channel is yours to protect — `deleteChannel` will refuse to delete the default channel unless you explicitly pass `forceDeleteIfDefault: true`, since a deleted default channel can't automatically reappear.

**`contentId`** is your own stable identifier for a piece of content (e.g. your backend's movie/episode ID) — every method that looks up, updates, or removes a program does so by matching this field, not by any system-generated ID.
...
Lists every channel belonging to your app.
...
| Type | Description |
|---|---|
| `ProgramType` | `MOVIE`, `TV_EPISODE`, `TV_SERIES`, `CLIP` |
| `ReviewRatingStyle` | `STARS`, `THUMBS_UP_DOWN`, `PERCENTAGE` |
| `GenreType` | Canonical genre tags (`COMEDY`, `DRAMA`, `SPORTS`, etc.) |
| `AspectRatio` | `RATIO_16_9`, `RATIO_1_1`, `RATIO_2_3`, `RATIO_3_2`, `RATIO_3_4`, `RATIO_4_3`, `MOVIE_POSTER` |
| `Availability` | `AVAILABILITY_AVAILABLE`, `AVAILABILITY_FREE`, `AVAILABILITY_FREE_WITH_ADS`, `AVAILABILITY_FREE_WITH_SUBSCRIPTION`, `AVAILABILITY_PAID_CONTENT`, `AVAILABILITY_PURCHASED` |
| `ContentRatingSystem` | Country-specific rating system identifiers (`US_TV`, `US_MV`, `KR_TV`, `AU_TV`, etc.) |
| `ContentRating` | `{ ratingSystem, rating, subRatings?, domain? }` |
| `ProgramData` | Full metadata object accepted by `addProgramToWatchNext` / `addProgramToChannel` — see inline JSDoc in `types.ts` for every field, including TV-episode-specific (`episodeTitle`, `seasonNumber`, etc.) and clip/series-specific fields |
| `CreateChannelData` | `{ displayName, appLinkUri, description?, appLinkText?, appLinkColor?, appLinkIconUri?, appLinkPosterArtUri?, displayOrder?, searchable? }` |
| `UpdateChannelData` | `Partial<CreateChannelData>` |
| `DeleteChannelOptions` | `{ channelId, forceDeleteIfDefault? }` |
| `TvChannel` | Shape returned by `getChannels()` |

> **Important field-naming gotcha:** for `type: ProgramType.TV_EPISODE`, the `title` field must hold the **series name** (e.g. "Breaking Bad"), not the episode's own name — the episode's own name goes in `episodeTitle`. This mirrors Android's own underlying data model and is easy to get backwards; getting it wrong doesn't throw an error, it just mislabels the card on screen.
...
MIT © [Daniel Obiekwe](https://github.com/MadeByRaymond)

## Support

If this package saved you from writing raw `androidx.tvprovider` Kotlin yourself, consider buying me a coffee:

[![Buy Me A Coffee](https://cdn.buymeacoffee.com/buttons/v2/default-yellow.png)](https://www.buymeacoffee.com/madebyraymond)
